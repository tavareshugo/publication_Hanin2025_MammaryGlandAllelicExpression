...
# reorder object to have alternating strains for each replicate
dds <- dds[, order(dds$cell_type, dds$timepoint, dds$animal_id, dds$allele_strain, dds$allele_parent)]
...
for (i_cell in unique(dds$cell_type)){
  for (i_timepoint in unique(dds$timepoint)) {
    ...
    # test
    test_results <- isolde_test(bias = "strain",
                                method = "default",
    ...
    file.rename(outfile, paste0("results/ase_isolde/", i_cell, "_", i_timepoint, "_isolde_strain.tsv"))
  }
}
...
for (i_cell in unique(dds$cell_type)){
  for (i_timepoint in unique(dds$timepoint)) {
    ...
    res <- read.table(paste0("results/ase_isolde/", i_cell, "_", i_timepoint, "_isolde_strain.tsv"),
                      header = TRUE)
    ...
    unlink(paste0("results/ase_isolde/", i_cell, "_", i_timepoint, "_isolde_strain.tsv"))
  }
}
...
write.csv(compiled_results,
          file = "results/ase_isolde/isolde_all_strain.csv",
          row.names = FALSE)
