...
for (i_cell in unique(dds$cell_type)){
  for (i_timepoint in unique(dds$timepoint)) {
    ...
    test_results <- isolde_test(bias = "parental",
                                ...
                                outdir = "results/ase_isolde_isoform/")

    # rename output files to remove date
    outfile <- list.files("results/ase_isolde_isoform/", pattern = paste0(i_cell, "_", i_timepoint, "_ALL"),
                          full.names = TRUE)
    if (length(outfile) != 1) stop("problem with output file")
    file.rename(outfile, paste0("results/ase_isolde_isoform/", i_cell, "_", i_timepoint, "_isolde.tsv"))
  }
}
...
for (i_cell in unique(dds$cell_type)){
  for (i_timepoint in unique(dds$timepoint)) {
    ...
    res <- read.table(paste0("results/ase_isolde_isoform/", i_cell, "_", i_timepoint, "_isolde.tsv"),
                      header = TRUE)
    ...
    unlink(paste0("results/ase_isolde_isoform/", i_cell, "_", i_timepoint, "_isolde.tsv"))
  }
}
...
write.csv(compiled_results,
          file = "results/ase_isolde_isoform/isolde_isoform_all.csv",
          row.names = FALSE)
