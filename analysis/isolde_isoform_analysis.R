...
# deseq dataset
dds <- readRDS("results/DESeqDataSet/dds_isoform_regular.rds")

# fetch gene annotation
annot <- dds %>% rowData() %>% as_tibble(rownames = "transcript")

# isolde data
isolde <- read_csv("results/ase_isolde_isoform/isolde_isoform_all.csv") %>%
...
expr <- dds[unique(c(isolde$transcript)), ] %>%
  assay("vst") %>%
...
summarised_expr <- expr %>%
  group_by(transcript, cell_type, stage, timepoint) %>%
  summarise(mean_expr = mean(expr), median_expr = median(expr))

# allele-specific counts
dds_isolde <- readRDS("results/DESeqDataSet/dds_isoform_allele.rds")
...
isolde %>%
  ...
  labs(x = "Number of conditions a transcript has ASE",
...
wrap_plots(
  isolde %>%
    ...
    annotation_logticks(sides = "l") +
    labs(x = "",
         y = "count", subtitle = "All transcripts flagged as ASE by ISOlDE"),
  isolde %>%
...
biotype_counts <- isolde %>%
  group_by(transcript, transcript_biotype) %>%
...
biotype_counts %>%
  ...
  mutate(transcript_biotype = transcript_biotype %>% str_replace_all("_", " ")) %>%
  ggplot(aes(pct_ase, transcript_biotype)) +
  geom_col()
...
biotype_counts %>%
  ...
  summarise(n_ase = sum(ase),
            ...
            pct_ase = mean(ase)*100) %>%
  filter(n_ase > 0) %>%
  mutate(transcript_biotype = fct_reorder(transcript_biotype, pct_ase)) %>%
  ggplot() +
  geom_point(aes(pct_ase, transcript_biotype, colour = "Observed"), size = 2) +
  geom_linerange(data = permutation_summary,
                 aes(xmin = lo, xmax = hi, y = transcript_biotype, colour = "95% \nexpected\nrange"),
                 size = 3, alpha = 0.2) +
  scale_colour_manual(values = c("steelblue", "black")) +
  labs(x = "% ASE transcripts", y = "Transcript biotype", colour = "",
...
ase_transcripts <- isolde %>%
  filter(status == "ASE" & chromosome_name != "Y" & abs(diff_prop) > 0.7) %>%
  group_by(transcript) %>% filter(n() >= 2) %>% ungroup() %>%
  distinct(transcript) %>% pull(transcript)

# ASE genes in 4 or more conditions
isolde %>%
  filter(status == "ASE" & chromosome_name != "Y") %>%
  group_by(transcript) %>% filter(n() > 3) %>% ungroup() %>%
...
# Gnas
isolde %>% filter(transcript %in% ase_transcripts) %>%
  filter(ensembl_gene_id == "ENSMUSG00000027523") %>%
  count(cell_type, is.na(diff_prop))

# Gnas overall expression
expr %>%
  left_join(annot, by = "transcript") %>%
  filter(ensembl_gene_id == "ENSMUSG00000027523") %>%
  ...
  facet_grid(cell_type ~ transcript) +
...
isolde %>%
  filter(ensembl_gene_id == "ENSMUSG00000027523") %>%
  # group_by(transcript) %>% filter(!all(is.na(diff_prop))) %>% distinct(transcript)
  ggplot(aes(stage, diff_prop)) +
  geom_hline(yintercept = 0) +
  geom_line(aes(colour = cell_type, group = cell_type)) +
  # geom_linerange(aes(ymin = diff_prop - variability, ymax = diff_prop + variability)) +
...
expr %>%
  left_join(annot, by = "transcript") %>%
  filter(ensembl_gene_id == "ENSMUSG00000048583") %>%
  mutate(cell_type = str_replace(cell_type, " ", "\n")) %>%
  ggplot(aes(stage, expr)) +
  ggbeeswarm::geom_quasirandom(aes(colour = stage)) +
  geom_line(stat = "summary", fun = "median", aes(group = 1)) +
  facet_grid(cell_type ~ transcript) +
...
expr %>%
  left_join(annot, by = "transcript") %>%
  filter(ensembl_gene_id == "ENSMUSG00000040856") %>%
  ggplot(aes(stage, expr)) +
  ggbeeswarm::geom_quasirandom(aes(colour = stage)) +
  geom_line(stat = "summary", fun = "median", aes(group = 1)) +
  facet_grid(cell_type ~ transcript) +
...
isolde %>%
  filter(ensembl_gene_id == "ENSMUSG00000040856") %>%
  group_by(transcript) %>% filter(!all(is.na(diff_prop))) %>%
  ggplot(aes(stage, diff_prop)) +
  geom_hline(yintercept = 0) +
  geom_line(aes(colour = cell_type, group = cell_type)) +
  # geom_linerange(aes(ymin = diff_prop - variability, ymax = diff_prop + variability)) +
